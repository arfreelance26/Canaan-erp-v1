...
const TriggerInput = forwardRef<HTMLInputElement, React.InputHTMLAttributes<HTMLInputElement>>(
  ({ value, onClick, onChange, placeholder, required, disabled }, ref) => (
    <div
      ...
      className={`relative flex w-full items-center rounded-lg border-2 border-brand-gold bg-white/80 px-3 py-2 backdrop-blur-sm transition-all duration-200 ${disabled ? "cursor-not-allowed opacity-60" : "cursor-pointer focus-within:bg-white focus-within:shadow-[0_0_0_4px_rgba(201,162,39,0.2)] hover:bg-white/90"}`}
    >
      <input
        ref={ref}
        type="text"
        ...
        required={required}
        disabled={disabled}
        className={`w-full bg-transparent text-sm uppercase text-gray-900 outline-none placeholder:normal-case placeholder:text-gray-400 ${disabled ? "cursor-not-allowed" : "cursor-pointer"}`}
      />
