...
function AttendanceRadioGroup({
  driverId, record, onMark,
}: { driverId: string; record: DriverAttendanceRecord | undefined; onMark: DriverAttendanceTableProps["onMark"]; }) {
        ...
        return (
          <label key={value} className={cn(
            ...
            <input type="radio" name={`attendance-${driverId}`} value={value} checked={checked}
              disabled={saving} onChange={() => handleSelect(value)} className="sr-only" />
...
function RemarkRow({
  remark, onUpdate, onDelete,
}: { remark: DriverAttendanceRemark; onUpdate: (id: string, text: string) => Promise<void>; onDelete: (id: string) => Promise<void>; }) {
...
function ViewAllDialog({
  driver, date, remarks, onClose, onAdd, onUpdate, onDelete,
}: {
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {remarks.length === 0 ? (
            ...
            <ul className="flex flex-col gap-3">
              {remarks.map((r) => (
                <RemarkRow key={r.id} remark={r} onUpdate={onUpdate} onDelete={onDelete} />
              ))}
            </ul>
          )}
        </div>
...
function RemarksCell({
  driver, date, remarks, onAdd, onUpdate, onDelete,
}: {
  driver: Driver; date: string; remarks: DriverAttendanceRemark[];
  onAdd: (driverId: string, remark: string) => Promise<DriverAttendanceRemark>;
  onUpdate: (id: string, text: string) => Promise<void>;
...
// ── Main table ────────────────────────────────────────────────────────────────
export function DriverAttendanceTable({ drivers, records, remarks, date, onMark, onAddRemark, onUpdateRemark, onDeleteRemark }: DriverAttendanceTableProps) {
  if (drivers.length === 0) {
    return (
