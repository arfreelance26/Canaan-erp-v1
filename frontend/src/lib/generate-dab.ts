...
export function buildDABHtml(data: DABData): string {
  const amtNum        = parseFloat(data.amount) || 0;
  const litresDisplay = data.litres ? `${data.litres} Litres` : "—";
...
<head>
<meta charset="UTF-8">
<title>Diesel Advance Voucher - Canaan Global International</title>
<style>
...
<div class="voucher-card">
  ...
  <div class="voucher-banner">DIESEL ADVANCE VOUCHER</div>
    ...
    <tbody>
      <tr>
        <td class="particulars-cell text-left">
          <div class="bold">Diesel Advance Adjustment</div>
          <div style="font-size:11px;color:#555;margin-top:4px;">
            Diesel supplied against advance for Trip Ref: ${esc(data.refNo || data.tripId)}
          </div>
        </td>
        <td class="particulars-cell text-center bold">${esc(litresDisplay)}</td>
    ...
    <div class="left-details">
      <div class="structured-box">
        ...
        <div class="box-grid">
          ...
          <div><span class="text-muted">Diesel Qty:</span> <span class="bold">${esc(litresDisplay)}</span></div>
...
export async function generateDABPdf(data: Omit<DABData, "logoSrc">, tripId: string): Promise<void> {
  ...
  const htmlContent = buildDABHtml({ ...data, logoSrc });
